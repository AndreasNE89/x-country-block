...
- New brand masters in `brand/`, store art and screenshot 5 in `store/`,
  all regenerated by `node scripts/render-brand.mjs`. Store screenshots 1,
  3 and 4 are rendered from the production popup by
  `node scripts/store-screenshots.mjs`; screenshot 2 is captured on x.com
